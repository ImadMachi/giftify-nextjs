	...
	return (
		<div className="flex items-center max-md:ml-auto md:space-x-6 space-x-2">
			...
			<div className="hidden md:inline link relative show-account p-1">
				<p className="text-xs text-slate-300">Hello, {session ? session.user?.name : "sign in"}</p>
					...
					{session ? (
						<div className="flex items-center justify-between p-3 border-b pb-2">
							...
							<div className="flex space-x-2">
								<Link href="/profile">
									<div className="button-orange px-6 py-[0.3rem] text-sm text-gray-900">Profile</div>
								</Link>
								<button onClick={() => signOut()} className="button-orange px-2 py-[0.3rem] text-sm text-gray-900">
									Sign Out
								</button>
							</div>
						</div>
						...
						<div className="flex flex-col items-center p-3 m-3 border-b pb-2">
							<button onClick={() => signIn()} className="button-orange px-16 py-[0.3rem] text-sm text-gray-900">
								Sign in
							</button>
							<p className="text-xs text-gray-900 mt-2">
								New customer?{" "}
								<Link href="/auth/register" className="text-[#05a] hover:text-amazon-orange hover:underline">
									start here
								</Link>
							</p>
						</div>
