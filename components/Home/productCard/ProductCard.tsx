...
const ProductCard = ({ product }: any) => {
	...
	const [styles, setStyles] = useState(product.subProducts.map((p: any) => p.color));

	useEffect(() => {
		setImages(product.subProducts[active]?.images);
		setPrices(product.subProducts[active]?.sizes.map((s: any) => s.price).sort((a: any, b: any) => a - b));
	}, [active]);
		...
		<div className="flex flex-col relative w-[215px] rounded p-1">
			<Link
				href={`/product/${product.slug}?style=${active}${`${
					product.subProducts[active].sizes.length > 1 ? "&size=" + active : ""
				}`}`}
			>
			...
			<div className=" mt-2">
				<Link href={`/product/${product.slug}?style=${active}`}>
					<h3>{product.name.length > 45 ? `${product.name.substring(0, 45)}` : product.name}</h3>
				</Link>
				<span className="text-xs text-red-500">
					{prices.length === 1 ? `USD${prices[0]}$` : `USD${prices[0]} - ${prices[prices.length - 1]}$`}
				</span>
